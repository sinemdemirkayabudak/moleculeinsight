...
def compute_fingerprints(query_df, ref_df, radius):
    ...
    try:
        query_df["fp"] = query_df["mol"].apply(lambda m: get_morgan_fp(m, radius))
        ref_df["fp"] = ref_df["mol"].apply(lambda m: get_morgan_fp(m, radius))
...
def search_similarities(query_df, ref_df):
    ...
    try:
        ...
        for query_name, query_fp in zip(query_df["query_name"], query_df["fp"], strict=True):
        ...
        logger.debug(
            f"Total comparisons: {len(combined_results)} ({len(query_df)} queries × {len(ref_df)} references)"
        )
...
def rank_results(combined_results, query_df, top_n):
    ...
    try:
        ...
        ranked_results = combined_results.sort_values(
            ["query_name", "similarity"], ascending=[True, False]
        )
