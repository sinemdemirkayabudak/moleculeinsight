...
from app.similarity_search import create_structure_image, run_similarity_search, prepare_csv_export
...
def display_results_table(results_df: pd.DataFrame) -> None:
    ...
    with col1:
        st.caption("💡 Tip: Click on an image to see a zoomed-in view of the query and reference molecules")
    ...
    display_df = results_df.drop(
        columns=['Reference SMILES', 'Query SMILES'],
        errors='ignore'
    )
    st.dataframe(
        display_df,
        column_config={
            "Structures": st.column_config.ImageColumn(
                "Structures",
                width="medium"
            )
        },
...
def generate_structure_images(results_df: pd.DataFrame, query_smiles_map: Dict[str, str]) -> List[Optional[str]]:
...
def prepare_display_dataframe(results_df: pd.DataFrame, query_smiles_map: Dict[str, str], structure_images: List[Optional[str]]) -> pd.DataFrame:
...
def process_similarity_results(results_df: pd.DataFrame, query_df: pd.DataFrame) -> pd.DataFrame:
    ...
    query_smiles_map = dict(zip(query_df_display['query_name'], query_df_display['smiles']))
...
def render_similarity_search() -> None:
    ...
    radius = st.sidebar.number_input("Fingerprint Radius", min_value=0, max_value=5, value=2, 
    ...
    with st.expander("View CSV Format Examples"):
        ...
        with col2:
            ...
            ref_example = pd.DataFrame({
                "smiles": ["CCO", "CCOC", "CCCC", "CC(=O)OC", "c1ccccc1O", "CC(=O)N", "CCCN", "COC", "CO"],
                "name": ["Ethanol_ref", "EthylMethylEther", "Butane", "MethylAcetate", "Phenol", "Acetamide", "Propylamine", "DimethylEther", "Methanol"]
            })
